"""Back up the live ledger online, then install the manual quota update."""
import os,shutil,sqlite3,subprocess,time,urllib.request,json
from pathlib import Path
base=Path('/opt/codex-relay-v2'); old=(base/'current').resolve()
release=base/'releases/manual-quota-20260905'
backup=Path('/opt/codex-relay-backups/manual-quota-'+time.strftime('%Y%m%d-%H%M%S'))
...
print('MANUAL_QUOTA_DEPLOYED',str(backup))
